// server/api/v1/categories/[id].ts
...
export default defineEventHandler(async (event) => {
  ...
  try {
    logger.debug("V1 Category By ID API request", {
      requestId,
      path: event.path,
      method: event.method,
      categoryId,
    ...
    // 標準化されたレスポンス形式を使用
    const response = formatApiResponse(category, requestId);
    ...
    // その他の予期しないエラー
    logger.error(
      "Error in v1 category by ID API",
      {
        requestId,
        path: event.path,
        method: event.method,
